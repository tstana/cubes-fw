...
#include "../msp/msp_configuration.h"
...
/* Static variable for a quick lookup table */
static unsigned long table_crc32[256];
static unsigned char have_table_crc32 = 0;

/* Calculate CRC32 table. */
static void calculate_table_crc32()
{
    int i, j;
    unsigned long rem;
    for (i = 0; i < 256; i++) {
...
unsigned long crc32(const unsigned char *data, unsigned long len, unsigned long start_remainder)
{
    unsigned long crc;
    unsigned long i;
    if (have_table_crc32 == 0)
        calculate_table_crc32();
    ...
    for (i = 0; i < len; i++)
        crc = (crc >> 8) ^ table_crc32[data[i] ^ (crc & 0xff)];
...
/* A pre-computed CRC32 table
static unsigned long table32[256] = {
